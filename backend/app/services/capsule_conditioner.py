"""Real-world camera conditioner and background isolator for Capsule inspection.

Adapts arbitrary phone camera, webcam, and desk photos of pharmaceutical capsules
to match the MVTec AD studio dark backdrop ([12, 12, 14]) distribution used by
EfficientAD and PatchCore specialists.
"""
...
def _find_capsule_salient_box(small_gray: Image.Image) -> Tuple[int, int, int, int] | None:
    """Find the central capsule/pill structure using edge detection and saliency weighting."""
    ...
    # Center weighting prior
    Y, X = np.ogrid[:gh, :gw]
    cx, cy = gw / 2.0, gh / 2.0
    dist_sq = ((X - cx) / (gw * 0.38)) ** 2 + ((Y - cy) / (gh * 0.38)) ** 2
    center_weight = np.exp(-0.5 * dist_sq)
    weighted = edge_arr * center_weight

    thresh = float(np.percentile(weighted, 80))
    if thresh < 2.0:
        return None
        ...
        for x in range(gw):
            if active[y, x] and not visited[y, x]:
                ...
                if len(comp) >= 12:
                    ...
                    score = len(comp) / max(1.0, cent_dist)
    ...
    if len(components) > 1 and components[1][0] > 0.45 * components[0][0]:
        b2 = components[1][1]
        top_box = (
            min(top_box[0], b2[0]),
            min(top_box[1], b2[1]),
...
def condition_capsule_input(image: Image.Image) -> Tuple[Image.Image, dict[str, Any]]:
    """Isolate, center, and background-neutralize a Capsule for real-world photos.

    Preserves standard MVTec AD images untouched while neutralizing surrounding
    desk/paper clutter and harsh desk-lamp reflections.
    """
    ...
    bbox_small = _find_capsule_salient_box(gray)
    ...
    bw = max(28, bbox_orig[2] - bbox_orig[0])
    bh = max(28, bbox_orig[3] - bbox_orig[1])
    cx = (bbox_orig[0] + bbox_orig[2]) // 2
    cy = (bbox_orig[1] + bbox_orig[3]) // 2

    # Canonical capsule extent
    capsule_extent = max(bw, bh)
    side = int(round(capsule_extent * 1.40))
    side = max(side, 64)
    ...
    # Soft radial vignette for capsule boundary
    rx = max(16.0, bw * 0.68)
    ry = max(16.0, bh * 0.68)
    r_inner = 1.02
    r_outer = 1.30
    ...
    conditioned = Image.fromarray(blended)
